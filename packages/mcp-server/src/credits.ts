...
const FREE_TIER_RESET_DAY = 1;

export const TOOL_PRICES: Record<string, ToolLimit> = {
  bastion_simulate_transaction: { free_per_month: 100, price_per_call_usd: 0.10 },
  bastion_override_block: { free_per_month: 10, price_per_call_usd: 1.00 },
  bastion_update_policy: { free_per_month: 5, price_per_call_usd: 5.00 },
  bastion_circuit_breaker_toggle: { free_per_month: 3, price_per_call_usd: 10.00 },
};
...
export function useFreeCall(agentId: string, toolName: string): { ok: true; remaining: number } | { ok: false; reason: string; price_usd: number } {
  ...
  return {
    ok: false,
    reason: `Free tier exhausted (${credits.count}/${limit} calls this month). Payment required via USDT/USDC.`,
