...
contract BastionERC8004Registry is ERC721URIStorage, EIP712, Ownable, IIdentityRegistry {
    ...
    constructor(
        address _owner
    ) ERC721("Bastion Agent", "BASTION") EIP712("ERC8004", "1") Ownable(_owner) { }
    ...
    function setAgentWallet(
        ...
        bytes32 structHash =
            keccak256(abi.encode(_SET_AGENT_WALLET_TYPEHASH, agentId, newWallet, deadline));
    ...
    function nextAgentId() external view returns (uint) {
        return _nextAgentId;
    }
    ...
    function globalAgentId(
        ...
        return string(
            abi.encodePacked(
                "eip155:", _toString(block.chainid), ":", _toChecksumAddress(address(this))
            )
        );
    }
