...
///    structurally different guarantees into one shared registry without this tag lets them get
///    silently conflated into a single green checkmark. This standard is `MECHANISM_ZK_SECRET_POLICY`.
struct VerdictAttestation {
    uint agentId; // ERC-8004 Identity Registry token id
...
library PolicyAttestation {
    ...
    function attestationFor(
        Verdict memory v
    ) internal pure returns (VerdictAttestation memory) {
...
interface IValidationRegistry {
    event VerdictRecorded(uint indexed agentId, bytes32 indexed nullifier, bytes32 mechanism);

    function recordVerdict(
        VerdictAttestation calldata attestation
    ) external;
}
