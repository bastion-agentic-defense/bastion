...
/// @dev Conforming Bastion implementation of the ERC-8354 normative core.
contract BastionConfidentialVerdict is IConfidentialPolicyVerdict, EIP712, ERC165 {
    ...
    constructor(
        IPolicyDomainRegistry _registry
    ) EIP712("BastionConfidentialVerdict", "1") {
        registry = _registry;
    }
    ...
    function verify(
        ...
        try IVerifier(d.verifier).verifyProof(d.programKey, abi.encode(v), proof) returns (
            bool ok
        ) {
            return ok;
        } catch {
    ...
    function _consume(
        ...
        if (!registry.isRootAcceptable(v.domainId, v.policyRoot)) {
            revert PolicyRootRejected(v.policyRoot); // 6
        }
        if (!IVerifier(d.verifier).verifyProof(d.programKey, abi.encode(v), proof)) {
            revert InvalidProof(); // 7
        }
    ...
    function _requireExecutorAuthorized(
        Verdict calldata v,
        bytes memory executorAuth
    ) internal view {
