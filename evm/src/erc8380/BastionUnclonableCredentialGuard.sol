...
/// @title BastionUnclonableCredentialGuard
/// @notice Conforming Bastion implementation of the ERC-8380 normative core. Answers the
///         adversarial vectors with two properties that a separable `consume` primitive cannot:
...
contract BastionUnclonableCredentialGuard is IUnclonableCredential {
    ...
    mapping(uint => uint) public override highestIssuedIndex;
    ...
    /// @notice Nullifier = H(NULLIFIER_TAG, salt). The chain id MUST NOT be in this preimage.
    function computeNullifier(
        bytes32 salt
    ) external pure returns (bytes32) {
        return salt.computeNullifier();
    }

    /// @notice capabilityCommitment = H(CAPABILITY_TAG, salt, agentId, homeChainId, homeDomainId,
    ...
    function computeCapabilityCommitment(
        ...
        return salt.computeCapabilityCommitment(
            agentId, homeChainId, homeDomainId, capabilityIndex, actionCommitment
        );
    }
    ...
    function issue(
        ...
        if (capabilityIndex > highestIssuedIndex[agentId]) {
            highestIssuedIndex[agentId] = capabilityIndex;
        }
    ...
    function _buildPublicInputs(
        ...
        inputs[6] = bytes32(uint(uint160(cap.executor)));
    ...
    function execute(
        ...
        if (!issued[cap.capabilityCommitment]) {
            revert CommitmentNotIssued(cap.capabilityCommitment);
        }
